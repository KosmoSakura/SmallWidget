...
import widget.small.com.smallwidget.tools.NetworkUtil;
...
public class OkHttpClientManager {
    ...
    private OkHttpClientManager(boolean addSSL) {
        cache = new Cache(new File(Constants.SAVE_SD_FLODER), 10240 * 1024);
...
//                .addInterceptor(appIntercepter)
    ...
    public static OkHttpClientManager getInstance(boolean addSSL) {
        if (mInstance == null) {
            synchronized (OkHttpClientManager.class) {
                if (mInstance == null) {
                    mInstance = new OkHttpClientManager(addSSL);
                }
            }
        }
        return mInstance;
    ...
    private void _getAsyn(String url, final ResultCallback callback) {
        final Request request = new Request.Builder()
                ...
                .addHeader("TerminalVersion", "v" + Constants.clientVersion)
                .addHeader("TerminalSim", Constants.phoneMODEL)
                .build();
...
//                    String fileName = getFileName(url) + System.currentTimeMillis() * 1024 / 5 + ".png";
...
//            .addFormDataPart("moduleId", BaseApp.sp.getInt("id", -1) + "")
...
//        Debug.e("TerminalVersion::" + "v" + FrameWorkSetting.clientVersion + "\n\nTerminalSim::" + FrameWorkSetting.phoneMODEL);
        return new Request.Builder()
                .url(url)
                .addHeader("TerminalCode", "android_app")
                .addHeader("TerminalVersion", "v" + Constants.clientVersion)
    ...
    private Interceptor cacheIntercepter = new Interceptor() {
        ...
        public Response intercept(Chain chain) throws IOException {
            ...
            if (!NetworkUtil.isConnected()) {
                ToastUtil.CustomShort("网络不可用,开始加载本地缓存");
